...
import { asArray, normalizeSwissRounds } from './tournament/compatibility';
...
export const getSwissHeadToHead = (
    ...
    for (const round of safeRounds) {
        for (const match of round) {
            const players = asArray<string | 'BYE'>(match?.players);
            if (players[0] === 'BYE' || players[1] === 'BYE') continue;
            const hasA = players[0] === playerIdA || players[1] === playerIdA;
            const hasB = players[0] === playerIdB || players[1] === playerIdB;
            if (hasA && hasB) {
...
export const sortSwissPlayers = (players: SwissPlayer[], rounds: SwissMatch[][]): SwissPlayer[] => {
    ...
    return [...safePlayers].sort((a, b) => {
        ...
        const h2h = getSwissHeadToHead(safeRounds, a.studentId, b.studentId);
